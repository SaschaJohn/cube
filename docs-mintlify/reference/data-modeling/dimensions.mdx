...
Each link must have a `name`, a `label`, and a `url`. The `name` is used as an identifier
in the [synthetic dimension](#synthetic) name. The `url` is a SQL expression that constructs
the link URL. It can [reference][ref-references] column and dimension values, just like
the [`sql` parameter](#sql) or [`mask` parameter](#mask).
